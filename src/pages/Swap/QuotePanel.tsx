...
const QuotePanel: React.FC<QuotePanelProps> = ({ exchangeRate }) => {
  ...
  // Handle both quote and exchange rate data
  const isBridgeExchange = !!exchangeRate;

  const toTokenAmount = isBridgeExchange
        ...
        <div className="flex items-center gap-2 text-base font-semibold">
          {isBridgeExchange ? (
            <span className="text-white/90">
              1 {fromToken?.symbol} = {exchangeRate?.rate?.toFixed(6)}{" "}
              {toToken?.symbol}
            </span>
          ) : (
            <>
              <span className="text-white/90">
                1 {toToken?.symbol} ={" "}
                {Number(
                  ((Number(fromAmount) || 1) / toTokenAmount).toFixed(10)
                )}{" "}
                {fromToken?.symbol}
              </span>
          ...
          {isBridgeExchange ? (
            <>
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white/80">
                  {exchangeRate?.fees?.depositFee?.toFixed(6)}{" "}
                  {fromToken?.symbol}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-white/50">Withdrawal Fee</span>
                <span className="text-white/80">
                  {exchangeRate?.fees?.withdrawalFee?.toFixed(6)}{" "}
                  {toToken?.symbol}
                </span>
              </div>
              <div className="flex justify-between">
                ...
                <span className="text-white/80 font-mono">
                  {exchangeRate?.minAmount?.toFixed(6)} {fromToken?.symbol}
                </span>
              </div>
              {exchangeRate?.maxAmount && (
                <div className="flex justify-between">
                  <span className="text-white/50">Max Amount</span>
                  <span className="text-white/80 font-mono">
                    {exchangeRate.maxAmount.toFixed(6)} {fromToken?.symbol}
                  </span>
                </div>
              )}
              <div className="flex justify-between">
