...
const BridgeTransactionProgress: React.FC<BridgeTransactionProgressProps> = ({
  ...
  const getProgressStep = () => {
    if (bridgeTransaction.status === 'executed') return 5;
    if (bridgeTransaction.status === 'pending') {
      ...
      if (progress < 0.25) return 1; // Waiting (0-25%)
      if (progress < 0.5) return 2; // Confirming (25-50%)
      if (progress < 0.75) return 3; // Exchanging (50-75%)
